...
// Hardcoded auth groups — no CLI spawn needed, so the setup page loads instantly.
const AUTH_GROUPS = [
  {
...
// Instant status: returns immediately with hardcoded auth groups + config state.
...
// Slow details: version + channels help. Fetched in the background by the UI.
app.get("/setup/api/status/details", requireSetupAuth, async (_req, res) => {
